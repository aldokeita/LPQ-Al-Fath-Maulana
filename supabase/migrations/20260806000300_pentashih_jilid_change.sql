-- Let a Pentashih change the jilid of santri in the classes they manage.
--
-- change_santri_jilid was gated on is_admin() OR (is_guru() AND
-- guru_has_santri_access()). is_guru() resolves user_profiles.role = 'guru'
-- exactly, so a teacher carrying the Pentashih role failed both branches: the
-- promote/demote buttons were visible but every attempt was rejected.
--
-- Scope stays narrow. pentashih_has_santri_access() requires an active
-- class_memberships row whose class has a current pentashih_class_assignments
-- entry with scope 'class' or 'both' for the caller, so this grants nothing
-- beyond the classes that Pentashih actually manages.
--
-- Only the authorization branch changes; the update, history write and error
-- contract are unchanged from 20260806000100.
...
begin
  ...
  if not (
    ...
    or (
      public.is_pentashih_user()
      and public.pentashih_has_santri_access(p_santri_id)
    )
  ) then
